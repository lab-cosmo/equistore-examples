...
class TripletEnergy(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        self.input_layer = torch.nn.Linear(
            in_features=self.input_size, out_features=1 )#self.intermediate_size)
...
#         layer_norm = torch.nn.LayerNorm(normalized_shape=self.intermediate_size)
#         inp = layer_norm(inp)
#         inp = torch.tanh(inp)
#         activation = self.hidden_layer(inp)
#         layer_norm = torch.nn.LayerNorm(normalized_shape=self.hidden_size)
#         activation= layer_norm(activation)
#         activation = torch.tanh(activation)
#         pred = self.out_layer(activation)
#         print(pred.shape)
        pred = self.model(x)
...
#checkpoint = torch.load('./models/triple_p0-nl-best.pt')
