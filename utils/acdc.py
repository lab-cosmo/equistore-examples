...
def cg_combine(x_a, x_b, m_a=None, m_b=None, M=None, feature_names=None, clebsch_gordan=None):
    """
...
def cg_increment(x_nu, x_1, m_nu=None, m_1=None, M=None, clebsch_gordan=None):
    ...
    return cg_combine(x_nu, x_1, m_nu, m_1, feature_names = feature_names, M = M, clebsch_gordan=clebsch_gordan)
...
def threshold_indices(x_a, x_b, m_a=None, m_b=None, sel_threshold=0.0, l_threshold=None, M = None, feature_names=None):
    """ 
    ...
    for index_a, block_a in x_a:
        ...
        for index_b, block_b in x_b:
            ...
            for i_a, f_a, bsz_a in zip(range(len(block_a.features)), block_a.features, block_a_sz.T):
                f_a = tuple(f_a)
                if m_a is None:
                    w_a = 1.0
                else:
                    w_a = m_a.block(index_a).values[0,0,i_a]
                for i_b, f_b, bsz_b in zip(range(len(block_b.features)), block_b.features, block_b_sz.T):
                    f_b = tuple(f_b)
                    if m_b is None:
                        w_b = 1.0
                    else:
                    ...
                    if ab_norm > sel_threshold:
                        for L in range(np.abs(lam_a-lam_b), 1+min(lam_a+lam_b,l_threshold) ):
                            S = sigma_a*sigma_b*(-1)**(lam_a+lam_b+L)
                            M_dict[(S,L,NU)][IDX] = W                            
                    else:
                        thresh_norm += ab_norm
