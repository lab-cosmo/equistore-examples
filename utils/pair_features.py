...
def rho0ij_builder(hypers, frames):
    ...
    gij_expansion = []
    for ijf in ijframes:
        gij_expansion.append(
            calculator.transform(ijf)
            .get_features(calculator)
            .reshape(len(ijf), max_atoms, hypers_ij["max_radial"], -1)
        )
    ...
    feat_builder = TensorBuilder(
        ["block_type", "L", "nu", "sigma", "species_i", "species_j"],
        ["structure", "center_i", "center_j"],
        [["mu"]],
        ["n"],
    )
    ...
    for sp_i in actual_global_species:
        for sp_j in actual_global_species:
            species_center_mask = np.where(info[:, 2] == sp_i)[0]
            species_neighbor_mask = np.where(info[:, 2] == sp_j)[0]
            for i, (struct_i, atom_i) in enumerate(
                info[species_center_mask[:]][..., :-1]
            ):
                for j, (struct_j, atom_j) in enumerate(
                    info[species_neighbor_mask[:]][..., :-1]
                ):
                    ifr = struct_i
                    ...
                    for l in range(lmax + 1):
                        ...
                        if block_idx not in feat_builder.blocks:
                            TensorBlock = feat_builder.add_block(
                                ...
                                properties=np.asarray(
                                    [list(range(hypers["max_radial"]))], dtype=np.int32
                                ).T,
                                components=[
                                    np.asarray(
                                        [list(range(-l, l + 1))], dtype=np.int32
                                    ).T
                                ],
                            )
                        ...
                        block_data = gij_expansion[struct_i][
                            atom_i % len_frames[ifr],
                            atom_j % len_frames[ifr],
                            :,
                            lm_slices[l],
                        ].T  # TODO: change for not water
                        ...
                        if block_type == 1:
                            if (atom_i % len_frames[ifr]) <= (atom_j % len_frames[ifr]):
                                block_data_ji = gij_expansion[struct_i][
                                    atom_j % len_frames[ifr],
                                    atom_i % len_frames[ifr],
                                    :,
                                    lm_slices[l],
                                ].T
                                TensorBlock.add_samples(
                                    labels=np.asarray(
                                        [
                                            [
                                                struct_i,
                                    ...
                                    data=(block_data + block_data_ji).reshape(
                                        (1, -1, block_data.shape[1])
                                    )
                                    / np.sqrt(2),
                                )
                            ...
                            TensorBlock.add_samples(
                                ...
                                data=block_data.reshape(1, -1, block_data.shape[1]),
                            )
    #
...
def tensor_g_rho_nu(rho0ij, rhoinu, hypers, cg, property_names=None):
        ...
        for index_b, block_b in rhoinu:
            ...
            for L in range(np.abs(lam_a - lam_b), min(lam_a + lam_b, lmax) + 1):
                ...
                for n_a in range(len(block_a.properties)):
                    f_a = tuple(
                        block_a.properties[n_a]
                    )  # values of n_a'th feature in block_a.features
                    for n_b in range(len(block_b.properties)):
                        f_b = tuple(
                            block_b.properties[n_b]
                        )  # values of n_b'th feature in block_b.features
                ...
                if block_type == 0:
                    ...
                    for samplea in block_a.samples:
                        ...
                        idxb = block_b.samples[
                            np.where(block_b.samples["center"] == centeri)
                        ]
                        samples_idxb = block_b.samples[
                            np.where(np.in1d(block_b.samples, idxb))[0]
                        ]
                        xx.append(
                            samples_idxb[np.where(samples_idxb["structure"] == stra)[0]]
                        )
                        if block_type == 1 or block_type == -1:
                            idxbj = block_b.samples[
                                np.where(block_b.samples["center"] == centerj)
                            ]
                            samples_idxbj = block_b.samples[
                                np.where(np.in1d(block_b.samples, idxbj))[0]
                            ]
                            yy.append(
                                samples_idxbj[
                                    np.where(samples_idxbj["structure"] == stra)[0]
                                ]
                            )
                    #     yy.append()
                    ...
                    if block_type == 1 or block_type == -1:
                        ...
                        if block_type == 1:
                            one_shot_blocks = (
                                one_shot_blocks_ij + one_shot_blocks_ji
                            ) / np.sqrt(2)
                        elif block_type == -1:
                            one_shot_blocks = (
                                one_shot_blocks_ij - one_shot_blocks_ji
    ...
    for block_idx in X_blocks:
        ...
        newblock = TensorBlock(
            ...
            properties=Labels(
                property_names, np.asarray(X_idx[block_idx], dtype=np.int32)
            ),
        )
    ...
    X = TensorMap(
        Labels(rho0ij.keys.names, np.asarray(nonzero_idx, dtype=np.int32)),
        nonzero_blocks,
    )
