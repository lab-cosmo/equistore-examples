    ...
    for A in range(len(frames)):
        ...
        for i, ai in enumerate(frame.numbers):
            ...
            for j, aj in enumerate(frame.numbers):
                ...
                if block_type == 1:
                    block_data_plus = (block_data + np.conjugate(block_data.T)) *1/np.sqrt(2)
                    block_data_minus = (block_data - np.conjugate(block_data.T)) *1/np.sqrt(2)
                ki_offset = 0
...
def blocks_to_dense(blocks, frames, orbs):
    ...
    for f in frames:
        ...
        for ai in f.numbers:
            norbs += orbs_tot[ai]
        ham = np.zeros((norbs, norbs), dtype=np.complex128)
        dense.append(ham)
        ...
        for (A,i,j), block_data in zip(block.samples, block.values):        
            ...
            if block_type == 0:
                ...
                if ki_offset != kj_offset:
                    ham[jslice, islice] = np.conjugate(block_data[:,:,0].reshape(2*li+1,2*lj+1).T)
#                     print(ham[jslice, islice], ham[islice, jslice])
            elif block_type == 2:
                ham[islice, jslice] = block_data[:,:,0].reshape(2*li+1,2*lj+1)
                ham[jslice, islice] = np.conjugate(block_data[:,:,0].reshape(2*li+1,2*lj+1).T )           
            elif block_type == 1:
                ham[islice, jslice] += np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1)  / np.sqrt(2), dtype=np.complex128)
                ham[jslice, islice] += np.conjugate(np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1).T / np.sqrt(2), dtype=np.complex128))
                if ki_offset != kj_offset:
                    islice = slice(ki_base+kj_offset, ki_base+kj_offset+2*lj+1)
                    jslice = slice(kj_base+ki_offset, kj_base+ki_offset+2*li+1)
                    ham[islice, jslice] += np.conjugate(np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1).T  / np.sqrt(2), dtype=np.complex128))
                    ham[jslice, islice] += np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1)  / np.sqrt(2), dtype=np.complex128)
            elif block_type == -1:
                ham[islice, jslice] += np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1) / np.sqrt(2), dtype=np.complex128)
                ham[jslice, islice] += np.conjugate(np.asarray(block_data[:,:,0].reshape(2*li+1,2*lj+1).T / np.sqrt(2), dtype=np.complex128))
                if ki_offset != kj_offset:
